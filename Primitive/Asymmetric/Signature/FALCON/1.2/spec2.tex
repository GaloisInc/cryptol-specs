...
\begin{code}
  // Check if a complex number is erroneous (NaN -- Not a Number).
  CHasNaN : C -> Bit
  CHasNaN x = fpIsNaN x.0 \/ fpIsNaN x.1

  // Check if an FFT value has any invalid entries.
  FFTHasNaN : {n} (fin n) => FFT n -> Bit
  FFTHasNaN = any CHasNaN

  /**
   * The split and merge functions are inverses of each other.
   * Section 4.1 claims that FALCON does not exercise "special case" FFT values,
   * including infinites or NaNs; NaN is explicitly excluded here to achieve
   * `:check`ability.
   *
   * ```repl
   * :check FFTInvFFT
   ...
   */
  FFTInvFFT : (FFT 4) -> Bit
  property FFTInvFFT f = ~ (FFTHasNaN f) ==> mergefft`{2,4} (splitfft`{2,4} f) == f
\end{code}
