...
\begin{code}
  ...
  BytesToBits2 input = [ (input@(i/8))@(i%8) | i <- [0 .. (ell*8-1)]]
  ...
  BitsBytesCorrectness : [12]Byte -> Bit
  BitsBytesCorrectness betas = BitsToBytes (BytesToBits betas) == betas

  BytesBitsCorrectness : [12*8]Bit -> Bit
  BytesBitsCorrectness betas = BytesToBits (BitsToBytes betas) == betas
  ...
  ParseInf ([bi,bi1,bi2] # tailS) =
      if d1 < `q then
          ...
          if d2 < `q then
              ...
              ParseInf tailS
      where
          d1 = toInteger bi + 256 * (toInteger bi1 % 16)
          d2 = floor(ratio (toInteger bi1) 16) + 16 * toInteger bi2
\end{code}
...
\begin{code}
  ...
  EncodeBytes' B = map reverse (groupBy (join B))

  DecodeBytes' : {ell} (fin ell, ell > 0) => [32 * ell]Byte -> [n][ell]
  DecodeBytes' B = groupBy (join (map reverse B))
  ...
  EncodeBytes B = join (map EncodeBytes' B)
  
  DecodeBytes : {ell, k1} (fin ell, ell > 0, fin k1) =>
      [32 * ell * k1]Byte -> [k1][n][ell]
  DecodeBytes B = map DecodeBytes' (split B)
...
\begin{code}
  ...
  ZBitsEquiv : (Z q) -> Bit
  ZBitsEquiv fi = (BitstoZ (ZtoBits`{12} fi) == fi)
  ...
  DecodeEf : {ell} (fin ell, ell > 0) => [32 * ell]Byte -> R_q
  DecodeEf B = map BitstoZ`{ell} (split (BytesToBits B))

  DecodeEquiv : [32 * 12]Byte -> Bit
  DecodeEquiv B = (Decode' B == DecodeEf B)
...
\begin{code}
  ...
  KeyGen(d) = (pk_rev#rho,sk_rev) where
    (rho,sigma) = G(d)
...
\begin{code}
  ...
  Enc(pk, m, r) = c1_rev#c2_rev where
    // pk_rev = [reverse (pk@(i) : Byte) | i <- [0 .. 12*k*n/8-1]]
    that = Decode`{12} (take`{12*k*n/8} pk) : [k]R_q
    ...
    v = (NTTInv' (dotVecVec that rvechat)) + e2 + (DecompressM'`{1}(DecodeBytes'`{1} m))

    c1_rev = EncodeBytes`{d_u}(Compress`{d_u}(u))
