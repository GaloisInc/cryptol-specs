...
This section does not have a direct analogue in SP800-67, but showcases
properties alluded to in Section 3.4.2 of that document.
...
private
    ...
    countUnique:
        {a, n, ix}
        (fin n, n > 0, Eq a, Ring ix, Literal 1 ix) =>
        [n]a -> ix
    countUnique xs
        | n == 1 => 1
        | n > 1 => if elem (head xs) (tail xs)
                   then countUnique (tail xs)
                   else 1 + countUnique (tail xs)

    /**
     * Count the number of unique subkeys
     * generated by `KS` for a given key.
     */
    uniqueSubkeys: [64] -> [8]
    uniqueSubkeys k = countUnique (KS k)

    /**
     * Possibly weak keys produce only
     * four unique round keys.
     *
     * Source: SP800-67 Section 3.4.2.
     * ```
     * :prove possiblyWeakDES
     * ```
     */
    property possiblyWeakDES = all (\k -> uniqueSubkeys k == 4) possiblyWeakKeys

    /**
     * Weak keys make DES an involution.
     *
     * Source: Wikipedia
     *         Article: Data Encryption Standard
     *         Section: Minor Cryptanalytic Properties
     ...
     * ```
     */
    property weakDES p = all (\k -> encipher k (encipher k p) == p) weakKeys

    /**
     * Semi-weak keys cause DES encipherment and decipherment
     * to become related.
     *
     * Source: SP800-67 Section 3.4.2.
     * ```
     ...
     * ```
     */
    property semiWeakDES p = all (
        \(k0, k1) -> (encipher k0 p == decipher k1 p)
                     &&
                     (encipher k1 p == decipher k0 p)
                     ) semiWeakKeys
```
